...
### 18.15 ข้อจำกัดและการแก้ไขล่าสุด (บั๊กและเสถียรภาพ)

1. **การดึงรายการตอนของซีรีส์ (Episode Draft Fallback)**: ในกรณีที่ระบบไม่พบตอนย่อยจากการสแกนหน้าเว็บซีรีส์ แต่มีการตรวจพบ direct video source อย่างน้อย 1 รายการ ระบบจะสร้างตอนที่ 1 (EP.1) ให้เป็นดราฟต์ตั้งต้นโดยอัตโนมัติเพื่อให้แอดมินสามารถบันทึกข้อมูลได้ทันที (แก้ไขบั๊กการข้ามบล็อกสร้างตอนเนื่องจากเงื่อนไขซ้ำซ้อนใน `main.tsx`)
2. **การจำกัดเวลาของคำขอเชื่อมต่อสื่อ (Media Fetch Timeout)**: API สำหรับ proxy และตรวจสอบสุขภาพของ URL สื่อภายนอก (`/api/play-proxy` และ `/api/admin/source-health`) จะมีการกำหนดเวลาหมดอายุ (Timeout) ที่ 10-15 วินาที เพื่อป้องกันการแขวนของ Express backend process ในกรณีที่ URL ปลายทางตอบสนองช้า
3. **การกรองโฆษณาประเภทไฟล์ .mp4 (Ad Filtering Rule)**: ระบบจะไม่ดึงไฟล์มีเดียประเภท `.mp4` ที่มีความยาว (duration) น้อยกว่า 20 นาที (1200 วินาที) เข้ามาเป็น direct source ของภาพยนตร์โดยเด็ดขาด เนื่องจากมักจะเป็นโฆษณาคั่นหรือคลิปสั้นภายนอก ยกเว้นไฟล์ประเภทสตรีมมิ่ง `.m3u8` ที่ระบบอนุญาตให้นำเข้ามาได้ตามปกติ
4. **การปรับปรุงโฉมหน้าแรกและส่วนรับชมของหน้าบ้าน (Netflix/Viu Premium Revamp)**: ปรับปรุงโครงสร้างสไตล์ชีทและการตกแต่งหน้า Catalogs และ Watch Page ทั้งหมดให้ดูพรีเมียมยิ่งขึ้น สไตล์ Netflix/Viu เช่น เพิ่มเอฟเฟกต์ Glassmorphism ใน Topbar, การสเกลและยกขอบโปสเตอร์เรืองแสงสีแดง (Hover scale up), การเกลี่ยความมืด (Gradient Overlay) บนแบนเนอร์ Hero และลดความฉูดฉาดของช่องโฆษณา Ad Slots โดยใช้สีโทนเข้มกลมกลืนแทน
5. **การปิดโหมด SQLite WAL และบังคับเขียนลงไฟล์โดยตรง (SQLite Journal Mode = DELETE)**: เปลี่ยนจากโหมดเขียนแบบบันทึกประวัติ (Write-Ahead Logging หรือ WAL) เป็นโหมดเขียนแบบดั้งเดิม (DELETE mode) ในระหว่างรัน SQLite เพื่อป้องกันปัญหายอดฮิตที่ข้อมูลใหม่ถูกค้างอยู่ในไฟล์ `.db-wal` และไม่ถูกอัปเกรดเข้าไปในไฟล์ `.db` หลัก (เนื่องจาก git ไม่ได้ติดตามไฟล์ `.db-wal` ทำให้เมื่อ push ขึ้น GitHub ข้อมูลจึงไม่อัปเดตตาม)
6. **ระบบการดึงข้อมูลสำรองด้วย JSON (JSON Database Fallback)**: เพิ่มระบบตรวจสอบการโหลดโมดูล `better-sqlite3` หากรันบน Vercel หรือสภาพแวดล้อมที่ไม่สามารถโหลดโมดูล Native (C++) ได้สำเร็จและล้มเหลวขณะ import ระบบจะเปลี่ยนไปใช้ตัวอ่านข้อมูลจากไฟล์ JSON สำรองที่ถูกสร้างขึ้นในขั้นตอน Build (`videos.json` และ `series.json`) ทันที ทำให้ API ไม่ออฟไลน์/ล่ม (HTTP 500) และหน้าเว็บยังคงดึงข้อมูลไปแสดงผลได้อย่างถูกต้อง
7. **การตั้งค่าคุกกี้เซสชันที่ปลอดภัยบน HTTPS (Secure Session Cookies)**: เพิ่มการกำหนดคุณสมบัติ `; Secure` ให้กับคุกกี้เซสชันโดยอัตโนมัติหากรันในสภาพแวดล้อม Production (Vercel) เพื่อความเข้ากันได้กับโปรโตคอล HTTPS และป้องกันเบราว์เซอร์บล็อกคุกกี้เซสชันทำให้เข้าระบบหลังบ้านผ่าน Vercel ได้อย่างถูกต้อง
8. **การบังคับแพ็กไฟล์ข้อมูล JSON ใน Serverless Functions (Vercel includeFiles)**: ปรับโครงสร้างการอ้างอิงโฟลเดอร์เก็บข้อมูลหลักมาใช้ `process.cwd()` และเพิ่มการระบุ `includeFiles` สำหรับโฟลเดอร์ `data/**` ใน `vercel.json` เพื่อบังคับให้ Vercel คัดลอกและแพ็กไฟล์ฐานข้อมูล JSON (`users.json`, `videos.json`, `series.json`) เข้าไปใน Serverless API ทุกครั้งที่ทำการ Deploy
