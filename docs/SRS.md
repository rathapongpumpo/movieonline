...
5. **การจัดการฐานข้อมูล SQLite บน Vercel (Vercel Database Copy)**: เนื่องจากสภาพแวดล้อมระบบของ Vercel เป็นแบบอ่านอย่างเดียว (Read-only filesystem) และการเรียก Pragma WAL หรือการปรับปรุงตารางจะส่งผลให้เซิร์ฟเวอร์ล่ม (HTTP 500) จึงกำหนดให้ระบบคัดลอกไฟล์ `site-source-inspector.db` (และไฟล์ WAL/SHM ที่เกี่ยวข้อง) ไปไว้ที่ `/tmp` ก่อนเปิดการใช้งานฐานข้อมูลเมื่อตรวจพบว่ารันอยู่บน Vercel เพื่อแก้ไขปัญหาการล็อกอินล้มเหลวและให้ API ทำงานต่อได้สำเร็จ
